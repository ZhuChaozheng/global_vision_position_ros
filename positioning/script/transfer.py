...
def callback(data):
    ...
    for i in range(10):
        ...
        imu_msg.linear_acceleration.x = atof(car_data[i * 7 + 5])
        imu_msg.linear_acceleration.y = atof(car_data[i * 7 + 6])
        imu_msg.angular_velocity.z = atof(car_data[i * 7 + 7])
        imu_data_pub = rospy.Publisher(
            ('robot_'+str(i+1)+'/imu'), Imu, queue_size=100)
        imu_data_pub.publish(imu_msg)
        # publish odometry
        od_msg.linear.x = atof(car_data[i * 7 + 1])
        od_msg.angular.z = atof(car_data[i * 7 + 2])
        ...
        encoder_msg.fr_speed_rpm = atof(car_data[i * 7 + 4])
        encoder_msg.fl_speed_rpm = atof(car_data[i * 7 + 3])
